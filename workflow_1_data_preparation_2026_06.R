...
# Eddy covariance workflow part 1/4 

# developed for LTAR UNL site based on original copy
# https://github.com/lsigut/EC_workflow 
# This code primarily aims at data preparation for quality control (QC). Meteo data
...
settings_file <- here("ltar_unl_settings_2026_06.R")
...
# eddypro_files <- read_EddyPro(paths$qc_input_eddypro, 
...
names(eddypro_2024) <- correct(names(eddypro_2024))

# Correct units
openeddy::units(eddypro_2024) <- 
  correct(openeddy::units(eddypro_2024), attr = "units")
...
# Sam edits: Since I don't have VPD from meteological file, 
# I will only rename RH
# names(eddypro_2024)[names(eddypro_2024) %in% c("RH", "VPD")] <- c("RH_EddyPro", "VPD_EddyPro")
names(eddypro_2024)[names(eddypro_2024) %in% "RH"] <- "RH_EddyPro"
