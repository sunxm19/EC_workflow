...
ROI <- st_zm(st_read('./data/UNL Pasture.kml'))
tower <- st_zm(st_read('./data/LTAR-PRHPA EC Tower.kml'))
...
# Compute spherical intersections (more precise)
sf_use_s2(TRUE)
inter <- st_intersection(tr_sf,st_geometry(boundary_line))  # keep tr_sf as sf, not sfc

# For each azimuth (transect ID), keep only the closest intersection point
tower_geom <- st_geometry(tower)


# Distances of nearest intersections from tower location
dist <- st_distance(tower_geom, st_geometry(inter))

# Prepare the output data frame (only azimuths that had an intersection)
out <- data.frame(Azimuth = inter$ID, Fetch = as.numeric(dist[1, ]))
